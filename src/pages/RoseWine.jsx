...
const RoseWine = ({isMobileScreen}) => {
  
  const base =  'rgb(118, 29, 29)';
  const [maxPrice, setMaxprice] = useState(1000);
  const [sort, setSort] = useState('asc');
  const redCollection = ['Rose'];
  ...
  return (
    <div className="redwine container">
       {isMobileScreen && <div className="mobile-pageNav redNav">
          ...
          <ArrowForwardIosIcon sx={{fontSize:".7em"}}/>
          <Link className="navLink pageNav-link" to='/Collections/Rosè-Wine'>ROSÉ WINE</Link>
        </div>}

      <div className="redwine-left">

        <div className="pageNav">
          <Link className="navLink pageNav-link">HOME</Link> 
          <ArrowForwardIosIcon sx={{fontSize:".7em"}}/>
          <Link to='/Collections' className="navLink pageNav-link">COLLECTIONS</Link>
          <ArrowForwardIosIcon sx={{fontSize:".7em"}}/>
          <Link className="navLink pageNav-link">RED WINE</Link>
        </div>
      ...
      <div className="redwine-right catalog-right">
          ...
          <div className="products-banner rose-banner">
            <h1 className="catalog-right-title ">ROSÉ WINE</h1>
            <div className="underline catalog-underline"></div>
          </div>
          
          <div className="products-list">
            <List collection={redCollection}  isNew={isNew}  isTrending={isTrending} isFeatured={isFeatured} sale={sale}  sort={sort} maxPrice={maxPrice} slide={slide} isWidgetOpen={isWidgetOpen} widgetOpen={widgetOpen}/>
          </div>
